...
function TodoArrayFunctions() {
  ...
  return { addTodo, addProject, todoListCounter, removeTodo, completeTodo, todoListArray, todoListCompleted }
}
...
function UIFunctions() {
  ...
  function displayTodoList() {
    ...
    if (currentPage == 'Inbox') {
      ...
      todoArrayFunctions.todoListArray.forEach(array => array.forEach((item, index) => { if (item.dueDate == today && item.completed == false) displayTodo(item, index) }));
    } else if (currentPage == 'Upcoming') {
      todoArrayFunctions.todoListArray.forEach(array => array.forEach((item, index) => { if (item.dueDate !== today && typeof (item) == 'object' && item.completed == false) displayTodo(item, index) }));
